...
class TatkalBooker:
    ...
    def login(self):
        ...
        captcha_solved = self.captcha.solve_with_retry(
            driver=self.driver,
            captcha_img_locator=(By.CSS_SELECTOR, "app-captcha img.captcha-img"),
            captcha_input_locator=(By.CSS_SELECTOR, "input[formcontrolname='captcha']"),
            refresh_locator=(By.CSS_SELECTOR, "app-captcha .captcha-canvas span.fa-repeat, app-captcha .captcha-canvas a"),
            submit_callback=lambda: self.safe_click(
                (By.XPATH, "//button[contains(text(),'SIGN IN')]"), timeout=5
            )
        )

        if not captcha_solved:
            logger.warning("Auto CAPTCHA failed. Please solve manually in the browser.")
        ...
        try:
            ...
            try:
                ...
                logger.info("Retrying login with manual CAPTCHA...")
    ...
    def search_train(self):
        ...
        try:
            ...
            time.sleep(0.5)
            tatkal_opt = self.driver.find_element(
                By.XPATH, f"//li[contains(@class,'ui-dropdown-item')]//span[contains(text(),'TATKAL') or contains(text(),'Tatkal')]"
            )
            tatkal_opt.click()
        except Exception as e:
    ...
    # ------------------------------------------------------------ #
    def run(self):
        """Execute the full tatkal booking flow."""
        try:
            # Determine tatkal window time
            tatkal_type = self.cfg.get("tatkal_type", "NON_AC").upper()
            if tatkal_type == "AC":
                tatkal_hour, tatkal_min = 10, 0  # 10:00 AM IST
            else:
                tatkal_hour, tatkal_min = 10, 0  # 10:00 AM for Non-AC (was 11, now both at 10)
